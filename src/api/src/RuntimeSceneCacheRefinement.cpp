...
void validate_policy(const BimCacheChunkingPolicy& policy) {
    ...
    constexpr std::size_t kMinimumTriangleBytes =
        3 * 3 * sizeof(float) + 3 * sizeof(std::uint32_t);
...
struct ChunkBuilder {
    ...
    bool can_append_triangle(
        ...
        if (current_bytes > policy.max_gpu_buffer_bytes) return false;
        const auto available = policy.max_gpu_buffer_bytes - current_bytes;
        return extra_position_bytes <= available &&
            extra_index_bytes <= available - extra_position_bytes;
    }
...
std::vector<std::size_t> spatial_primitive_order(const BimCacheChunkDTO& source) {
    ...
    std::sort(order.begin(), order.end(), [&](std::size_t first, std::size_t second) {
        ...
        // Z-first keeps storey-scale imported proxy parts adjacent. X/Y then
        // preserve locality inside the floor so camera residency does not pull
        // unrelated corners of a campus into the same GPU chunk.
        if (a.z != b.z) return a.z < b.z;
        if (a.y != b.y) return a.y < b.y;
        if (a.x != b.x) return a.x < b.x;
        return source.primitives[first].element_id.value <
            source.primitives[second].element_id.value;
    });
    return order;
}
...
void refine_chunk(
    ...
    for (const auto primitive_index : spatial_primitive_order(source)) {
        ...
        while (consumed < index_count) {
            ...
            // Check the first triangle before recording this fragment's range.
            // If only the previous primitive consumed the remaining budget,
            // flush and retry with a fresh builder; this avoids carrying an
            // index offset from the old chunk into the new one.
            const auto first_triangle_offset = first_index + consumed;
            const auto first_triangle_a = source.indices[first_triangle_offset];
            const auto first_triangle_b = source.indices[first_triangle_offset + 1];
            const auto first_triangle_c = source.indices[first_triangle_offset + 2];
            if (!builder.can_append_triangle(
                    first_triangle_a,
                    first_triangle_b,
                    first_triangle_c,
                    policy
            ...
            while (consumed < index_count) {
                ...
                if (!builder.can_append_triangle(first, second, third, policy)) break;
            ...
            const auto fragment_index_count =
                builder.chunk.indices.size() - fragment_first_index;
            if (fragment_index_count == 0 || !fragment_has_bounds) {
                throw std::runtime_error("BIM cache refinement produced an empty primitive fragment");
            }
            ...
            builder.chunk.primitives.push_back(BimCachePrimitiveDTO{
                ...
                // Oversized primitives are normally imported proxies and carry
                // no architectural edge stream. Keep authored edges on one
                // fragment only so a split wall/proxy cannot double-render them.
                .feature_edges = first_fragment
                    ? primitive.feature_edges
                    : std::vector<RenderSceneFeatureEdgeDTO>{},
...
BimCacheSceneDTO compile_bounded(
    ...
    // Three arguments resolve to the seed compiler in RuntimeSceneCache.cpp.
