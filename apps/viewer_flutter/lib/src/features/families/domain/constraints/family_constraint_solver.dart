...
abstract final class FamilyConstraintSolver {
  ...
  static FamilyConstraintSolution solveSketch(
    ...
    for (final resolved in constraints) {
      final constraint = resolved.source;
      if (constraint.kind != FamilySketchConstraintKind.pointOnReferencePlane) {
        continue;
      }
      ...
      if (plane.axis == FamilyReferencePlaneAxis.x) {
        x.fix(resolved.a, offset, source: plane.name, tolerance: _tolerance);
      } else {
        y.fix(resolved.a, offset, source: plane.name, tolerance: _tolerance);
      }
    }
  ...
  static List<FamilySketchPoint> _solveStage2(
    ...
    for (var iteration = 0; iteration < _stage2Iterations; iteration++) {
      for (final constraint in constraints) {
        _applyStage2(points, constraint, targets[constraint.source.id]);
      }
