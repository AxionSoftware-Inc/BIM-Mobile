...
final class FamilyMeshVertex {
  ...
  FamilyMeshVertex copyWith({double? x, double? y, double? z}) =>
      FamilyMeshVertex(x: x ?? this.x, y: y ?? this.y, z: z ?? this.z);
}
...
abstract final class FamilyGeometryEvaluator {
  ...
  static FamilyEvaluatedMesh _fitMeshToTypeParameters(
    ...
    return mesh.copyWith(vertices: List<FamilyMeshVertex>.unmodifiable(vertices));
  }
  ...
  static FamilyEvaluatedMesh _booleanMesh(
    ...
    if (feature.kind == FamilyFeatureKind.booleanUnion) {
      return _mergeMeshes(left, right, source: source)
          .copyWith(isApproximate: true);
    }
    ...
    required String source,
      ...
      FamilyEvaluatedMesh(
        vertices: List<FamilyMeshVertex>.unmodifiable(
          mesh.vertices.map(
            (vertex) => FamilyMeshVertex(x: vertex.x, y: vertex.y, z: vertex.z),
          ),
        ),
      ...
      faces: List<FamilyMeshFace>.unmodifiable(
        <FamilyMeshFace>[
          ...
          for (final face in right.faces)
            FamilyMeshFace(
              List<int>.unmodifiable(face.indices.map((index) => index + offset)),
            ),
        ],
      ),
      source: source,
  ...
  static FamilyPreviewShape _evaluateFeature(
    ...
    switch (feature.kind) {
      ...
      case FamilyFeatureKind.extrude:
        return _profileFeatureShape(document, type, feature) ??
  ...
  static FamilyEvaluatedMesh _boxMesh(
    ...
    FamilyTypeDefinition type,
      ...
      _extrudeMesh(_boxShape(document, type), _boxShape(document, type).source);
  ...
  static FamilyEvaluatedMesh _extrudeMesh(
    ...
    for (var index = 0; index < count; index++) {
      ...
      faces.add(FamilyMeshFace(<int>[index, next, count + next, count + index]));
    }
    return FamilyEvaluatedMesh(
  ...
  static FamilyEvaluatedMesh _revolveMesh(
    ...
    if (!fullRevolution) {
      faces.add(FamilyMeshFace(List<int>.generate(profile.length, (index) => index)));
  ...
  static FamilyEvaluatedMesh _transformMesh(
    ...
    final translateX = _resolveScalar(
      feature.parameters['translationX'], document, type, fallback: 0);
    final translateY = _resolveScalar(
      feature.parameters['translationY'], document, type, fallback: 0);
    final translateZ = _resolveScalar(
      feature.parameters['translationZ'], document, type, fallback: 0);
    ...
    final scale = _resolveScalar(
      feature.parameters['scale'], document, type, fallback: 1);
  ...
  static int? _inputOrPreviousSolidIndex(
    ...
    for (final input in feature.inputs.reversed) {
      final inputIndex = document.features.indexWhere((candidate) => candidate.id == input);
  ...
  static List<int> _booleanInputIndices(
    ...
    for (var cursor = index - 1; cursor >= 0 && result.length < 2; cursor--) {
      if (_isSolidFeature(document.features[cursor]) && !result.contains(cursor)) {
        result.add(cursor);
      }
    }
  ...
  static String _featureName(FamilyFeatureKind kind) => switch (kind) {
