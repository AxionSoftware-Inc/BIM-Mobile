...
export '../constraints/family_constraint_models.dart';
...
final class FamilySketchPoint {
  ...
  FamilySketchPoint copyWith({String? id, double? x, double? y}) {
    return FamilySketchPoint(id: id ?? this.id, x: x ?? this.x, y: y ?? this.y);
  }
...
final class FamilySketch {
    ...
    final normalized = <FamilySketchPoint>[
      for (var index = 0; index < requested.length; index++)
        requested[index].id.trim().isNotEmpty
            ? requested[index]
            : index < this.points.length && this.points[index].id.trim().isNotEmpty
  ...
  static FamilySketch? fromJson(Object? raw) {
    ...
    final plane = _enumFromName(FamilySketchPlane.values, raw['plane']?.toString());
    ...
    if (rawPoints is List) {
      for (var index = 0; index < rawPoints.length; index++) {
        ...
        points.add(point.id.trim().isEmpty ? point.copyWith(id: '$id:point-$index') : point);
      }
    }
    return FamilySketch(
      id: id,
...
final class FamilyParameterDefinition {
  ...
  static FamilyParameterDefinition? fromJson(Object? raw) {
    ...
    final kind = _enumFromName(FamilyParameterKind.values, raw['kind']?.toString());
...
final class FamilyTypeDefinition {
  ...
  FamilyTypeDefinition copyWith({String? name, Map<String, Object?>? values}) {
  ...
  Map<String, Object?> toJson() => <String, Object?>{'id': id, 'name': name, 'values': values};
...
final class FamilyFeature {
  ...
  static FamilyFeature? fromJson(Object? raw) {
    if (raw is! Map) return null;
    final id = raw['id']?.toString().trim() ?? '';
    final kind = _enumFromName(FamilyFeatureKind.values, raw['kind']?.toString());
...
final class FamilyDocument {
  ...
  factory FamilyDocument.starter({
    ...
    const parameters = <FamilyParameterDefinition>[
      FamilyParameterDefinition(id: 'width', label: 'Width', kind: FamilyParameterKind.length, defaultValue: 1.0, minimum: 0.01),
      FamilyParameterDefinition(id: 'depth', label: 'Depth', kind: FamilyParameterKind.length, defaultValue: 1.0, minimum: 0.01),
      FamilyParameterDefinition(id: 'height', label: 'Height', kind: FamilyParameterKind.length, defaultValue: 1.0, minimum: 0.01),
    ];
    return FamilyDocument(
      ...
      types: const <FamilyTypeDefinition>[
        FamilyTypeDefinition(id: 'type-1', name: 'Default Type', values: <String, Object?>{'width': 1.0, 'depth': 1.0, 'height': 1.0}),
      ],
      features: const <FamilyFeature>[
        FamilyFeature(id: 'feature-1', kind: FamilyFeatureKind.box, parameters: <String, Object?>{'width': 'width', 'depth': 'depth', 'height': 'height'}),
      ],
    );
  }
  ...
  FamilyDocument copyWith({
    ...
    return FamilyDocument(
      ...
      parameters: List<FamilyParameterDefinition>.unmodifiable(parameters ?? this.parameters),
      ...
      referencePlanes: List<FamilyReferencePlane>.unmodifiable(referencePlanes ?? this.referencePlanes),
      constraints: List<FamilySketchConstraint>.unmodifiable(constraints ?? this.constraints),
  ...
  Map<String, Object?> toJson() => <String, Object?>{
        ...
        if (referencePlanes.isNotEmpty) 'reference_planes': referencePlanes.map((item) => item.toJson()).toList(),
        if (constraints.isNotEmpty) 'constraints': constraints.map((item) => item.toJson()).toList(),
      };
  ...
  static FamilyDocument? fromJson(Object? raw) {
    ...
    if (schemaVersion < minimumSupportedSchemaVersion || schemaVersion > currentSchemaVersion) return null;
    final id = raw['id']?.toString().trim() ?? '';
    final name = raw['name']?.toString().trim() ?? '';
    final category = _enumFromName(FamilyCategory.values, raw['category']?.toString());
    ...
    final sketchById = <String, FamilySketch>{for (final sketch in sketches) sketch.id: sketch};
    ...
    return FamilyDocument(
      ...
      referencePlanes: List<FamilyReferencePlane>.unmodifiable(referencePlanes),
...
FamilySketchConstraint _hydrateConstraintPointIds(FamilySketchConstraint constraint, FamilySketch? sketch) {
  String? idAt(int? index, String? existing) {
    if (existing?.trim().isNotEmpty == true) return existing!.trim();
    if (sketch == null || index == null || index < 0 || index >= sketch.points.length) return null;
