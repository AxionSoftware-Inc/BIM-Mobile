...
final class FamilyParameterResolver {
  ...
  Object? resolve(FamilyParameterDefinition parameter) => resolveById(parameter.id);
...
final class _ExpressionParser {
  _ExpressionParser(this.source, {required this.resolveIdentifier});
  ...
  String? _identifier() {
    _skipWhitespace();
    if (_offset >= source.length ||
        !_isIdentifierStart(source.codeUnitAt(_offset))) {
      return null;
    }
    final start = _offset++;
