...
typedef FamilyCandidateSceneLoader = Future<RenderScene> Function(
  FamilyDocument document,
  FamilyTypeDefinition type, {
  required Iterable<String> featureIds,
});

/// Family viewport host that reuses the production project RenderScene viewport.
///
/// Presentation owns no library/storage lookup. Candidate-scene loading is
/// injected by the composition boundary so this widget can be reused with a
/// local library, cloud library, tests, or an in-memory document set.
class FamilyAuthoringViewport extends StatefulWidget {
  const FamilyAuthoringViewport({
    ...
    required this.type,
    required this.mesh,
    required this.candidateSceneLoader,
  ...
  final FamilyCandidateSceneLoader candidateSceneLoader;
...
class _FamilyAuthoringViewportState extends State<FamilyAuthoringViewport> {
  ...
  Future<void> _configureAndLoad({required bool resetView}) async {
    ...
    final scene = widget.mode == FamilyAuthoringViewportMode.pickFeatures
        ? await widget.candidateSceneLoader(
            widget.document,
            widget.type,
