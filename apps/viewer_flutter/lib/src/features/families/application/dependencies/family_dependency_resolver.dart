...
/// Pure nested-family dependency resolver.
///
/// This application service has no file-system, Flutter, or library-storage
/// dependency. Callers provide the available family documents explicitly; an
/// infrastructure adapter is responsible for obtaining them from local/cloud
/// storage.
abstract final class FamilyDependencyResolver {
  ...
  static double _resolveScalar(
    ...
    if (raw is num) {
      ...
      if (!value.isFinite) {
        throw const FormatException(
          'Nested transform contains a non-finite value.',
        );
      }
      return value;
    }
