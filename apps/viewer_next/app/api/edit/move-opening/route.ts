...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const commandPath = path.join(workDir, "move_opening.json");
    ...
    return NextResponse.json({ success: true, request: payload, validation: result.validation ?? { issues: 0, warnings: 0, errors: 0 }, artifactPaths: result.artifact_paths, commandOutput: result, message: "Opening moved successfully.", stdout, stderr });
  } catch (error) {
